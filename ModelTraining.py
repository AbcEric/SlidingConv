...
# GPU:
# By default, tensorflow try to allocate a fraction per_process_gpu_memory_fraction of the GPU memory to his process
# to avoid costly memory management. This can fail and raise the CUDA_OUT_OF_MEMORY warnings.
# I do not know what is the fallback in this case (either using CPU ops or a allow_growth=True).
# This can happen if an other process uses the GPU at the moment
# The default behavior takes ~95% of the memory.
...
def read_and_decode(fileName):
    ...
    # 比较一下前后是否一致
    img = tf.reshape(img, [32,280,1])               # 为何变为[32, 280]?
    ...
    def predict_model(self, input_data):
        ...
        with self.session.as_default():
            with self.graph.as_default():
                ...
                print("result1=[", result_str, "]")
                result_str = self.adjust_label(decoded[1])
                print("result2=[", result_str, "]")
                return result_str
...
def main():
    ...
    if TRAINING:
        ...
        result_str = sc.predict_model(image)
